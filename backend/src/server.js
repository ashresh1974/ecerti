...
app.post('/api/register', async (req, res) => {
  ...
  try {
    ...
    const checkDuplicateSql = 
      'SELECT * FROM users WHERE username = ? OR email_id = ? OR roll_number = ? OR phone_number = ?';
    ...
    if (duplicateUsers.length > 0) {
      ...
      if (existing.phone_number === mobile) {
    ...
    const insertUserSql =
      'INSERT INTO users (full_name, roll_number, course, branch, phone_number, email_id, username, password) VALUES (?, ?, ?, ?, ?, ?, ?, ?)';
...
app.post('/api/register/send-otp', async (req, res) => {
  ...
  try {
    ...
    // Delete existing OTP
    const deleteExistingOtpSql = 'DELETE FROM otps WHERE email = ?';
    await db.query(deleteExistingOtpSql, [email]);

    // Insert new OTP
    const insertOtpSql = 'INSERT INTO otps (email, otp, expires_at) VALUES (?, ?, ?)';
    await db.query(insertOtpSql, [email, otp, expiresAt]);
...
app.post('/api/register/verify-otp', async (req, res) => {
  ...
  try {
    ...
    const deleteOtpSql = 'DELETE FROM otps WHERE id = ?';
    await db.query(deleteOtpSql, [results[0].id]);
...
app.post('/api/forgot-password/reset-password', async (req, res) => {
  ...
  try {
    ...
    await db.query(deleteOtpSql, [email]);
